...
parser.add_argument('--validation_length', default=230, type=int, help='Length of validation set')
parser.add_argument('--test_length', default=270, type=int, help='Length of testing set')
...
training_dataset_rolled = generate_rolling_dataset(dataset = train_dataset,
                                                   start_time=pd.Timestamp(df.index[0], freq="1H"),
                                                   end_time=pd.Timestamp(df.index[train_length],freq ="1H"),
...
for j,ts in enumerate(training_sets): #iterating over the rolling sets
        interim_dict = ListDataset(ts, freq="1H") #N times (1,truncated length)
...
#passive gain
passive_gain_train = 100*(df_open[pd.Timestamp(correct_training_dates[-1]).strftime('%Y-%m-%d %H:%M:%S')] - df_open[pd.Timestamp(correct_training_dates[0]).strftime('%Y-%m-%d %H:%M:%S')])/df_open[pd.Timestamp(correct_training_dates[0]).strftime('%Y-%m-%d %H:%M:%S')]
passive_gain_test = 100*(df_open[pd.Timestamp(correct_testing_dates[-1]).strftime('%Y-%m-%d %H:%M:%S')] - df_open[pd.Timestamp(correct_testing_dates[0]).strftime('%Y-%m-%d %H:%M:%S')])/df_open[pd.Timestamp(correct_testing_dates[0]).strftime('%Y-%m-%d %H:%M:%S')]
