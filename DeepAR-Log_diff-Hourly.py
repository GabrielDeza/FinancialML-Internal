...
parser.add_argument('--train_length', default=300, type=int, help='Length of training set')
parser.add_argument('--validation_length', default=350, type=int, help='Length of validation set')
parser.add_argument('--test_length', default=420, type=int, help='Length of testing set')
...
filename = f"/Users/gabriel/Desktop/Fixed_Stock_Project/ADSK_1h.csv"
...
training_data = ListDataset([{
        ...
        FieldName.FEAT_DYNAMIC_REAL: FDR}
            for (target, start,FDR) in zip(target_values, start_date, feat_dynamic)], freq ="H")
...
plotting_training_data = ListDataset([{
    ...
    FieldName.START: start,
    FieldName.FEAT_DYNAMIC_REAL: FDR}
        for (target, start, FDR) in zip(target_values, start_date, plotting_feat_dynamic)], freq ="H")
...
validation_data = ListDataset([{
        ...
        FieldName.START: start,
        FieldName.FEAT_DYNAMIC_REAL: FDR}
            for (target, start,FDR) in zip(validation_target_values, start_date, feat_dynamic_val)], freq ="H")
...
testing_data = ListDataset([{
        ...
        FieldName.START: start,
        FieldName.FEAT_DYNAMIC_REAL: FDR}
    for (target, start, FDR) in zip(testing_target_values, start_date,plotting_test_feat_dynamic)], freq ="H")
...
dataset_rolled = generate_rolling_dataset(
        ...
        start_time=pd.Timestamp(df_target.index[0], freq="H"),
        end_time=pd.Timestamp(df_target.index[train_length+prediction_length], freq = "H"),
...
for i,train_dict in enumerate(dataset_rolled):
    ...
    train_data = ListDataset([{
            ...
            FieldName.START: start,
            FieldName.FEAT_DYNAMIC_REAL: FDR}
                for (target, start, FDR) in zip(interim_target, start_, fdr)], freq ="H")
    mx.random.seed(seed)
    np.random.seed(seed)
...
dataset_rolled = generate_rolling_dataset(
        ...
        start_time=pd.Timestamp(df_target.index[validation_length], freq="H"),
        end_time=pd.Timestamp(df_target.index[test_length+prediction_length], freq = "H"),
        strategy=StepStrategy(prediction_length=prediction_length, step_size = step_size))
...
for i,(train_dict) in enumerate(dataset_rolled):
    ...
    train_data = ListDataset([{FieldName.TARGET: target,FieldName.START: start,FieldName.FEAT_DYNAMIC_REAL: FDR} for (target, start, FDR)
                              in zip(interim_target, start_, fdr)], freq ="H")
    mx.random.seed(seed)
    np.random.seed(seed)
    for test_entry, forecast in zip(train_data, predictor.predict(train_data)):
